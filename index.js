import{a as $,i as I,S as re}from"./assets/vendor-CYE3kvfv.js";(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))r(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const l of a.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&r(l)}).observe(document,{childList:!0,subtree:!0});function t(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function r(i){if(i.ep)return;i.ep=!0;const a=t(i);fetch(i.href,a)}})();const p={headerBtnOpenMenu:document.querySelector(".header-button-menu"),headerBtnCloseMenu:document.querySelector(".button-close"),mobileMenu:document.querySelector(".menu-drive")},F=()=>{p.mobileMenu.classList.toggle("is-open"),document.body.classList.toggle("no-scroll")};p.headerBtnOpenMenu&&p.mobileMenu&&p.headerBtnOpenMenu.addEventListener("click",F);p.headerBtnCloseMenu&&p.mobileMenu&&p.headerBtnCloseMenu.addEventListener("click",F);function oe(e){const s=Math.floor(e/1e3),t=Math.floor(s/60),r=s%60;return`${t}:${r.toString().padStart(2,"0")}`}function ne(e,s){return!e&&!s?"Information missing":s?`${e} - ${s}`:`${e}-present`}function ie(){const e=document.querySelector(".loader");e&&(e.style.display="block")}function ae(){const e=document.querySelector(".loader");e&&(e.style.display="none")}function ce(e,s=250){e||(e="Information missing");const t=e.length>s,r=e.slice(0,s);return`
  ...
  `}function le(e){const s=document.querySelector("#bio-toggle"),t=document.querySelector("#bio-text");if(!s)return;let r=!1;s.addEventListener("click",()=>{r=!r,r?(t.textContent=e,s.innerHTML=`
        <svg class="modal-icon" width="20" height="20">
      ...
      `)})}function de(){const e=document.querySelector(".modal-overlay"),s=document.querySelector(".modal"),t=document.querySelector(".btn-exit");if(!e||!s||!t){console.error("Modal elements not found in DOM");return}function r(){e.classList.remove("is-open")}t.addEventListener("click",r),e.addEventListener("click",i=>{i.target===e&&r()}),document.addEventListener("keydown",i=>{i.key==="Escape"&&r()})}const ue=$.create({baseURL:"https://sound-wave.b.goit.study/api"});async function pe(e){try{ie(),document.querySelector(".modal-overlay").classList.add("is-open");const{data:t}=await ue.get(`/artists/${e}/albums`);fe(t)}catch(s){console.error("Error loading artist:",s)}finally{ae()}}function fe(e){const{strArtistThumb:s,strArtist:t,intFormedYear:r,intDiedYear:i,genres:a,strGender:l,intMembers:n,strCountry:o,strBiographyEN:c,albumsList:u}=e,y=document.querySelector(".modal");y.innerHTML="";const ee=(a==null?void 0:a.map(d=>`<span>${d}</span>`).join(" "))||"",te=(u||[]).slice(0,8).map(({strAlbum:d,tracks:f})=>{const v=(f==null?void 0:f.map(({strTrack:q,intDuration:se,movie:D})=>`
          <li class="track">
            <span class="title">${q}</span>
            <span class="time">${oe(se)}</span>
      ...
      `}).reduce((d,f,v,q)=>(v%2===0?d.push(`<div class="albums-thumb">${f}`):d[d.length-1]+=f+"</div>",v===q.length-1&&v%2===0&&(d[d.length-1]+="</div>"),d),[]).join("");y.innerHTML=`
    ...
    <div class="actor">
      ...
      <ul class="actor-info">
        <div class="actor-info-container">
          <li><h2>Years active</h2><p>${ne(r,i)}</p></li>
  ...
  `,le(c),de()}const Y=document.querySelector(".artists-list"),z=document.querySelector(".artists-loader"),U=document.querySelector(".artists-load-more-btn");function J(){z.classList.remove("is-hidden")}function K(){z.classList.add("is-hidden")}function ge(){U.classList.remove("is-hidden")}function T(){U.classList.add("is-hidden")}function P(){Y.innerHTML=""}function me(e){return!Array.isArray(e)||e.length===0?"":`<div class="artist-genres">${e.map(t=>`<span class="genre-tag">${t}</span>`).join(" ")}</div>`}function W(e){const s=e.map(t=>{const r=t.strBiographyEN||"";return`
